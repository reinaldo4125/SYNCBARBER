...
export default function ClientDashboard({
  ...
  const [portalSubTab, setPortalSubTab] = useState<"citas" | "memberships" | "loyalty">("citas");
  ...
  const [timeSlotPeriod, setTimeSlotPeriod] = useState<"all" | "morning" | "afternoon" | "night">("all");
  const [isAutofilledFromCache, setIsAutofilledFromCache] = useState(false);
  ...
  // Autofill booking details if logged in OR from guest profile cache
  useEffect(() => {
    if (loggedClient) {
      ...
      setIsAutofilledFromCache(false);
    } else {
      const cachedGuest = localStorage.getItem("syncbarber_guest_client_data");
      if (cachedGuest) {
        try {
          ...
          if (parsed.birthDate) {
            setClientBirthDate(parsed.birthDate);
            setUserBirthDateInput(parsed.birthDate);
          }
          if (parsed.name || parsed.phone) {
            setIsAutofilledFromCache(true);
          }
        } catch (e) {
          console.error("Error loading cached guest data:", e);
        }
      }
    }
  }, [loggedClient]);
  ...
  // Set initial date automatically when dates are available
  useEffect(() => {
  ...
  const isSlotBusy = (date: string, time: string) => {
    ...
    const endA = startA + serviceDuration;
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newApp && newApp.id) {
        ...
        // Cache guest details for fast 1-click repeat bookings
        localStorage.setItem("syncbarber_guest_client_data", JSON.stringify({
          name: clientName,
          phone: clientPhone,
          email: clientEmail,
  ...
  return (
    <div className="space-y-6 sm:space-y-8 pb-24 sm:pb-8" id="client-dashboard">
      {/* Saludo Banner */}
      <div className="bg-gradient-to-r from-elegant-card to-elegant-sub text-elegant-text rounded-2xl sm:rounded-3xl p-4.5 sm:p-6 md:p-8 border border-elegant-border shadow-md relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sm:gap-6">
        ...
        <div className="space-y-2 relative z-10">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold tracking-tight font-sans text-white text-custom-brand">
            Bienvenido a {config.name}
          </h1>
          {config.tagline ? (
            <p className="text-xs sm:text-sm text-elegant-gold/90 font-medium italic max-w-lg">
              "{config.tagline}"
            </p>
          ) : (
            <p className="text-xs sm:text-sm text-elegant-text-muted max-w-lg leading-relaxed">
      ...
      {/* Navigation Tabs bar - 2 tabs: Agendar & Mi Portal */}
      <div className="grid grid-cols-2 sm:flex sm:items-center sm:gap-2 border-b border-elegant-border pb-1">
        <button
          onClick={() => setClientTab("booking")}
          className={`flex items-center justify-center gap-2 px-3 py-2.5 sm:px-6 sm:py-3 rounded-t-xl sm:rounded-t-2xl font-sans text-xs sm:text-sm font-semibold transition-all duration-200 border-b-2 text-center cursor-pointer ${
            clientTab === "booking"
        ...
        <button
          onClick={() => {
            setClientTab("account");
            setPortalSubTab("citas");
          }}
          className={`flex items-center justify-center gap-2 px-3 py-2.5 sm:px-6 sm:py-3 rounded-t-xl sm:rounded-t-2xl font-sans text-xs sm:text-sm font-semibold transition-all duration-200 border-b-2 text-center cursor-pointer ${
            clientTab === "account" || clientTab === "memberships"
              ? "bg-elegant-card text-elegant-gold border-elegant-gold"
              : "text-elegant-text-muted hover:text-white border-transparent hover:bg-elegant-sub/30"
          }`}
        >
          <User className="h-4 w-4 shrink-0" />
          <span className="truncate">{loggedClient ? `Mi Portal (${loggedClient.name.split(" ")[0]})` : "Mi Portal"}</span>
      ...
      {clientTab === "booking" && (
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-elegant-card border border-elegant-border rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-xs space-y-5 sm:space-y-6">
            <div className="border-b border-elegant-border pb-3 sm:pb-4">
              <h2 className="text-base sm:text-lg font-bold text-white font-sans flex items-center gap-1.5">
                <Scissors className="h-4.5 w-4.5 sm:h-5 sm:w-5 text-elegant-gold" />
                Nueva Reserva en Segundos
              </h2>
              <p className="text-xs text-elegant-text-muted mt-0.5">Sigue los pasos a continuación para agendar tu cita rápidamente.</p>
            </div>
            ...
            {bookingSuccess && (
              <div className="bg-emerald-950/40 border border-emerald-800/50 rounded-2xl p-5 text-center space-y-4 animate-scaleUp">
                ...
                {/* Botones de Exportar a Calendario Google & Apple */}
                {(() => {
                  const targetBarber = barbers?.find(b => b.id === bookingSuccess.barberId);
                  const calEventData = {
                    title: `Cita en ${config?.name || 'Peluquería'}: ${bookingSuccess.serviceName}`,
                    description: `Cita confirmada para ${bookingSuccess.serviceName} con ${bookingSuccess.barberName || 'Barbero'}.\nCliente: ${bookingSuccess.clientName}\nTeléfono: ${config?.phone || ''}`,
                    location: config?.address || config?.name || "Barbería / Salón",
                    startDate: bookingSuccess.date,
                    startTime: bookingSuccess.time,
                    durationMinutes: bookingSuccess.duration || 30
                  };
                  const gCalUrl = createGoogleCalendarUrl(calEventData);

                  return (
                    <div className="bg-neutral-900/80 border border-neutral-700/60 rounded-2xl p-3.5 max-w-sm mx-auto space-y-2.5 text-left">
                      <p className="text-[11px] font-bold text-elegant-gold uppercase tracking-wider flex items-center gap-1.5">
                        <CalendarPlus className="h-4 w-4" />
                        Agendar Recordatorio en tu Calendario:
                      </p>
                      <div className="grid grid-cols-2 gap-2">
                        <a
                          ...
                          className="flex items-center justify-center gap-1.5 px-3 py-2 bg-neutral-800 hover:bg-neutral-700 text-white rounded-xl text-xs font-bold transition-all border border-neutral-600 cursor-pointer text-center"
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => downloadIcsFile(calEventData)}
                          className="flex items-center justify-center gap-1.5 px-3 py-2 bg-neutral-800 hover:bg-neutral-700 text-white rounded-xl text-xs font-bold transition-all border border-neutral-600 cursor-pointer text-center"
                        >
            ...
            {!bookingSuccess && (
              <div className="space-y-6">
                ...
                {/* Stepper Visual de Pasos Interactivos con Navegación Rápida */}
                <div className="bg-neutral-900/95 border border-elegant-border rounded-2xl p-2 sm:p-3 shadow-inner">
                  <div className="grid grid-cols-4 gap-1.5 sm:gap-2 text-center text-xs">
                    {/* Paso 1 */}
                    <button
                      type="button"
                      onClick={() => scrollToStep("step-1-barber")}
                      className={`p-2 sm:p-2.5 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all cursor-pointer ${
                        selectedBarberId
                          ? "bg-amber-500/15 border-amber-500/50 text-amber-300"
                          : "bg-neutral-800/40 border-neutral-700/50 text-neutral-400 hover:text-white"
                      }`}
                    >
                      <div className="flex items-center gap-1">
                        <span className="w-4 h-4 sm:w-4.5 sm:h-4.5 rounded-full bg-amber-500 text-black font-mono font-black text-[10px] flex items-center justify-center">1</span>
                        <span className="font-bold text-[11px] sm:text-xs">Barbero</span>
                      </div>
                      <span className="text-[10px] sm:text-[11px] truncate max-w-full font-medium text-neutral-300">
                        {selectedBarberId === "any" ? "Cualquiera" : (barbers.find(b => b.id === selectedBarberId)?.name?.split(" ")[0] || "Elegido")}
                      </span>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => scrollToStep("step-2-datetime")}
                      className={`p-2 sm:p-2.5 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all cursor-pointer ${
                    ...
                    >
                      <div className="flex items-center gap-1">
                        <span className="w-4 h-4 sm:w-4.5 sm:h-4.5 rounded-full bg-amber-500 text-black font-mono font-black text-[10px] flex items-center justify-center">2</span>
                        <span className="font-bold text-[11px] sm:text-xs">Horario</span>
                      </div>
                      <span className="text-[10px] sm:text-[11px] truncate max-w-full font-medium text-neutral-300">
                        {selectedTime ? `${formatTime(selectedTime, config?.timeFormat)}` : "Elige"}
                      </span>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => scrollToStep("step-3-services")}
                      className={`p-2 sm:p-2.5 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all cursor-pointer ${
                        selectedService
                          ? "bg-amber-500/15 border-amber-500/50 text-amber-300"
                          : "bg-neutral-800/40 border-neutral-700/50 text-neutral-400 hover:text-white"
                    ...
                    >
                      <div className="flex items-center gap-1">
                        <span className="w-4 h-4 sm:w-4.5 sm:h-4.5 rounded-full bg-amber-500 text-black font-mono font-black text-[10px] flex items-center justify-center">3</span>
                        <span className="font-bold text-[11px] sm:text-xs">Servicio</span>
                      </div>
                      <span className="text-[10px] sm:text-[11px] truncate max-w-full font-medium text-neutral-300">
                        {selectedService ? selectedService.name.split(" ")[0] : "Elige"}
                      </span>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => scrollToStep("step-4-contact")}
                      className={`p-2 sm:p-2.5 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all cursor-pointer ${
                    ...
                    >
                      <div className="flex items-center gap-1">
                        <span className="w-4 h-4 sm:w-4.5 sm:h-4.5 rounded-full bg-emerald-500 text-black font-mono font-black text-[10px] flex items-center justify-center">4</span>
                        <span className="font-bold text-[11px] sm:text-xs">Datos</span>
                      </div>
                      <span className="text-[10px] sm:text-[11px] truncate max-w-full font-medium text-neutral-300">
                        {clientName ? clientName.split(" ")[0] : "Confirmar"}
                      </span>
                    </button>
                  </div>
                </div>
                ...
                <div id="step-1-barber" className="space-y-3.5 sm:space-y-4 scroll-mt-6">
                  <h3 className="text-xs sm:text-sm font-bold uppercase tracking-wider text-elegant-gold flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-3.5">
                    ...
                    <div
                      onClick={() => {
                        ...
                        setSelectedTime("");
                        setTimeout(() => scrollToStep("step-2-datetime"), 120);
                      }}
                      className={`border rounded-2xl p-3.5 sm:p-4 cursor-pointer transition-all flex items-center gap-3 ${
                    ...
                    >
                      <div className="h-11 w-11 sm:h-12 sm:w-12 rounded-xl bg-elegant-border/80 text-elegant-gold flex items-center justify-center font-bold text-xl border border-elegant-gold/30 shrink-0">
                        ✨
                      </div>
                      <div>
                        <h4 className="font-bold text-xs sm:text-sm text-white leading-tight">Cualquier Barbero</h4>
                        <p className="text-[11px] sm:text-xs text-elegant-text-muted mt-0.5">Asignación automática</p>
                        <span className="text-[10px] sm:text-[11px] text-emerald-400 font-semibold flex items-center gap-1.5 mt-1">
                    ...
                    {barbers.filter(b => b.isActive).map((barber) => {
                      ...
                      return (
                        <div
                          ...
                          className={`border rounded-2xl p-3.5 sm:p-4 cursor-pointer transition-all flex items-center gap-3 relative overflow-hidden ${
                            isBlockedToday
                        ...
                        >
                          <div className="relative shrink-0">
                            <div className={`h-11 w-11 sm:h-12 sm:w-12 rounded-xl border overflow-hidden flex items-center justify-center font-bold text-sm ${
                              isBlockedToday 
                            ...
                            {!isBlockedToday && (
                              <span className="absolute -bottom-0.5 -right-0.5 bg-emerald-500 h-2.5 w-2.5 sm:h-3 sm:w-3 rounded-full border-2 border-elegant-bg" title="Disponible" />
                            )}
                          </div>

                          <div className="flex-1 min-w-0">
                            <h4 className={`font-bold text-xs sm:text-sm truncate leading-tight ${isBlockedToday ? "text-rose-400" : "text-white"}`}>
                              {barber.name}
                            </h4>
                            {isBlockedToday ? (
                              <p className="text-[10px] sm:text-[11px] text-rose-400 font-bold mt-0.5">🔴 Ausente / Descanso</p>
                            ) : (
                              <>
                                <p className="text-[11px] sm:text-xs text-elegant-text-muted mt-0.5 truncate">
                                ...
                                <div className="flex items-center gap-1 mt-1 text-xs text-elegant-gold font-bold">
                                  ...
                                  <span className="text-elegant-text-muted text-[10px] sm:text-[11px] font-normal">
                                    ({ratingInfo.count})
                                  </span>
                                </div>
                              </>
                ...
                {/* Paso 2: Selección de Fecha y Hora (Agenda) */}
                <div id="step-2-datetime" className="space-y-4 sm:space-y-5 pt-5 border-t border-elegant-border animate-fadeIn scroll-mt-6">
                  <h3 className="text-xs sm:text-sm font-bold uppercase tracking-wider text-elegant-gold flex items-center gap-2">
                  ...
                  {/* Banner de Próximo Turno Disponible Rápido */}
                  {(() => {
                    ...
                    return (
                      <div className="bg-gradient-to-r from-amber-950/40 via-neutral-900 to-amber-950/40 border border-amber-500/40 rounded-2xl p-3 sm:p-3.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-amber-500/20 text-amber-300 flex items-center justify-center shrink-0 border border-amber-500/30">
                            <Zap className="h-4 w-4 sm:h-5 sm:w-5 text-amber-400 animate-pulse" />
                          </div>
                          <div>
                            <p className="text-xs font-bold text-white flex items-center gap-2">
                              <span>{isToday ? "⚡ Próximo Turno Hoy:" : "⚡ Primer Turno Libre:"}</span>
                              <strong className="text-amber-400 font-mono text-xs sm:text-sm bg-amber-500/20 px-2 py-0.5 rounded-lg border border-amber-500/30">{formatTime(earliest, config?.timeFormat)}</strong>
                            </p>
                            <p className="text-[10px] sm:text-[10.5px] text-neutral-400">Toca para agendar en 1-clic con la mayor rapidez</p>
                          </div>
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                            ...
                            setTimeout(() => scrollToStep("step-3-services"), 120);
                          }}
                          className="w-full sm:w-auto px-3.5 py-2 bg-amber-500 hover:bg-amber-400 text-black font-extrabold text-xs rounded-xl transition-all shadow-md cursor-pointer active:scale-95 flex items-center justify-center gap-1.5"
                        >
                          <span>Elegir {formatTime(earliest, config?.timeFormat)}</span>
                  ...
                  {/* Fecha de Agendamiento - Tira Horizontal Desplazable (Spacious swipe strip) */}
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2.5 overflow-x-auto pb-2.5 pt-1 scrollbar-none snap-x snap-mandatory -mx-1 px-1">
                      {availableDates.map((dateStr) => {
                        ...
                        return (
                          <button
                            ...
                            className={`min-w-[76px] xs:min-w-[84px] sm:min-w-[90px] p-3 rounded-2xl border flex flex-col items-center justify-center transition-all cursor-pointer snap-start shrink-0 ${
                              isSelected
                                ? "border-elegant-gold bg-elegant-gold text-elegant-bg font-bold shadow-lg shadow-amber-500/20 scale-[1.02]"
                                : "border-elegant-border bg-elegant-sub text-elegant-text hover:bg-elegant-card hover:border-neutral-600 active:scale-95"
                            }`}
                          >
                            <span className={`text-[11px] font-extrabold uppercase tracking-wider ${isSelected ? "text-elegant-bg" : "text-neutral-400"}`}>
                              {label.dayName.slice(0, 3)}
                            </span>
                            <span className="text-lg sm:text-xl font-black font-sans my-0.5">
                              {label.dayNum}
                            </span>
                            <span className={`text-[10px] sm:text-[11px] font-mono capitalize ${isSelected ? "text-elegant-bg font-bold" : "text-neutral-400"}`}>
                              {label.monthName}
                            </span>
                          </button>
                        );
                  ...
                  {/* Hora de Agendamiento con Bloques de Mañana / Tarde / Noche */}
                  <div className="space-y-3 pt-2">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                      ...
                      {/* Filtro por bloques de horario */}
                      <div className="flex items-center gap-1 bg-black/40 p-1 rounded-xl border border-neutral-800 text-xs overflow-x-auto scrollbar-none">
                        <button
                          type="button"
                          onClick={() => setTimeSlotPeriod("all")}
                          className={`px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer whitespace-nowrap ${
                            timeSlotPeriod === "all" ? "bg-amber-500 text-black shadow-xs" : "text-neutral-400 hover:text-white"
                          }`}
                        >
                          ✨ Todos
                        </button>
                        <button
                          type="button"
                          onClick={() => setTimeSlotPeriod("morning")}
                          className={`px-2 py-1 rounded-lg font-bold transition-all flex items-center gap-1 cursor-pointer whitespace-nowrap ${
                            timeSlotPeriod === "morning" ? "bg-amber-500 text-black shadow-xs" : "text-neutral-400 hover:text-white"
                          }`}
                          title="08:00 a 12:59"
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => setTimeSlotPeriod("afternoon")}
                          className={`px-2 py-1 rounded-lg font-bold transition-all flex items-center gap-1 cursor-pointer whitespace-nowrap ${
                            timeSlotPeriod === "afternoon" ? "bg-amber-500 text-black shadow-xs" : "text-neutral-400 hover:text-white"
                          }`}
                          title="13:00 a 17:59"
                        >
                          <CloudSun className="h-3 w-3" /> Tarde
                        </button>
                        <button
                          type="button"
                          onClick={() => setTimeSlotPeriod("night")}
                          className={`px-2 py-1 rounded-lg font-bold transition-all flex items-center gap-1 cursor-pointer whitespace-nowrap ${
                            timeSlotPeriod === "night" ? "bg-amber-500 text-black shadow-xs" : "text-neutral-400 hover:text-white"
                          }`}
                          title="18:00 en adelante"
                        >
                    ...
                    {(() => {
                      const filteredSlots = timeSlots.filter((slot) => {
                        if (timeSlotPeriod === "all") return true;
                        const [h] = slot.split(":").map(Number);
                        if (timeSlotPeriod === "morning") return h < 13;
                        if (timeSlotPeriod === "afternoon") return h >= 13 && h < 18;
                        if (timeSlotPeriod === "night") return h >= 18;
                        return true;
                      });

                      if (filteredSlots.length === 0) {
                        return (
                          <p className="text-xs text-neutral-400 py-4 text-center bg-neutral-900/50 rounded-xl border border-neutral-800">
                            No hay turnos disponibles en este bloque horario. Prueba con otro bloque o día.
                          </p>
                        );
                      }

                      return (
                        <div className="grid grid-cols-3 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 sm:gap-2.5">
                          {filteredSlots.map((timeStr) => {
                            ...
                            return (
                              <button
                                ...
                                className={`py-3 px-2 text-center font-mono font-bold text-xs xs:text-sm rounded-xl transition-all ${
                                  isBusy 
                                    ? "bg-elegant-sub/30 border border-elegant-border/30 text-neutral-600 cursor-not-allowed line-through" 
                                    : isSelected
                                      ? "bg-elegant-gold text-elegant-bg font-black ring-2 ring-elegant-gold shadow-md shadow-amber-500/10 scale-105"
                                      : "bg-elegant-gold/10 border border-elegant-gold/30 hover:bg-elegant-gold/25 hover:border-elegant-gold text-elegant-gold cursor-pointer active:scale-95"
                                }`}
                              >
                ...
                {/* Paso 3: Selección de Servicio o Corte */}
                {selectedDate && selectedTime && (
                  <div id="step-3-services" className="space-y-3.5 pt-5 border-t border-elegant-border animate-fadeIn scroll-mt-6">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-elegant-gold flex items-center gap-2">
                      ...
                      Selecciona el Servicio o Corte
                    </h3>

                    {/* Banner Lookbook: ¿No sabes qué corte hacerte? */}
                    <div className="bg-gradient-to-r from-neutral-900 via-elegant-card to-neutral-900 border border-elegant-gold/40 rounded-2xl p-4 sm:p-5 relative overflow-hidden shadow-lg">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3.5 sm:gap-4">
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 sm:h-11 sm:w-11 rounded-2xl bg-elegant-gold/20 border border-elegant-gold/40 flex items-center justify-center text-elegant-gold shrink-0 shadow-md">
                            <Camera className="h-5 w-5" />
                          </div>
                          <div className="space-y-0.5">
                            <div className="flex items-center gap-2">
                              <span className="text-[9px] sm:text-[10px] bg-elegant-gold text-black font-extrabold px-2 py-0.2 rounded uppercase tracking-wider">
                                ¿Indeciso?
                              </span>
                              <span className="text-[10px] sm:text-[11px] text-amber-400 font-bold flex items-center gap-1">
                                <Sparkles className="h-3 w-3" /> Catálogo de Estilos
                              </span>
                            </div>
                            ...
                            <p className="text-[11px] text-elegant-text-muted leading-relaxed">
                              Abre nuestro catálogo con fotos reales de Fades, Pompadour, Barbas y Diseños.
                            </p>
                          </div>
                        </div>
                      ...
                      {selectedCatalogStyle && (
                        <div className="mt-3.5 pt-3.5 border-t border-neutral-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-neutral-900/80 p-3 rounded-xl border border-amber-500/30">
                          ...
                          return (
                            <div
                              ...
                              onClick={() => {
                                ...
                                setTimeout(() => scrollToStep("step-4-contact"), 120);
                              }}
                ...
                {selectedService && selectedDate && selectedTime && (
                  <form onSubmit={handleBookingSubmit} id="step-4-contact" className="space-y-4 pt-5 border-t border-elegant-border animate-fadeIn text-sm scroll-mt-6">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-elegant-gold flex items-center gap-2">
                    ...
                    {/* Banner de Autocompletado Express de Visita Anterior */}
                    {isAutofilledFromCache && !loggedClient && (
                      <div className="bg-amber-950/30 border border-amber-500/30 p-3 rounded-2xl flex items-center justify-between gap-2 text-xs animate-fadeIn">
                        <div className="flex items-center gap-2">
                          <Zap className="h-4 w-4 text-amber-400 shrink-0" />
                          <span className="text-amber-200">
                            ⚡ <strong>Autocompletado Express:</strong> Datos cargados de tu visita anterior.
                          </span>
                        </div>
                        <button
                          type="button"
                          ...
                          className="text-[11px] text-neutral-400 hover:text-amber-300 underline font-medium cursor-pointer"
                        >
                          Limpiar / Cambiar
                        </button>
                      </div>
      ...
      {(clientTab === "account" || clientTab === "memberships") && (
        <div className="max-w-4xl mx-auto animate-fadeIn space-y-6">
          {loggedClient ? (
            ...
            <div className="bg-elegant-card border border-elegant-border rounded-3xl p-4 sm:p-6 space-y-6 shadow-xs">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pb-5 border-b border-elegant-border">
                <div className="flex items-center gap-3 text-center sm:text-left flex-col sm:flex-row">
                  <div className="h-14 w-14 bg-elegant-gold/10 text-elegant-gold border border-elegant-gold/20 rounded-full flex items-center justify-center font-sans font-extrabold text-lg shrink-0">
                    {loggedClient.name.slice(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <div className="flex items-center gap-2 justify-center sm:justify-start flex-wrap">
                      <h3 className="text-base sm:text-lg font-bold text-white font-sans">{loggedClient.name}</h3>
                      {loggedClient.membershipActive && (
                    ...
                    <div className="mt-1 flex flex-wrap items-center justify-center sm:justify-start gap-1.5 text-[11px] text-amber-300 font-mono">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      setClientTab("booking");
                      setTimeout(() => scrollToStep("step-1-barber"), 100);
                    }}
                    className="px-3.5 py-2 bg-elegant-gold hover:bg-elegant-gold-hover text-elegant-bg font-extrabold rounded-xl text-xs flex items-center gap-1.5 cursor-pointer transition-all shadow-xs"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-2 border border-rose-900/50 hover:bg-rose-950/20 text-rose-400 font-bold rounded-xl text-xs flex items-center gap-1 cursor-pointer transition-colors"
                  >
              ...
              {/* Sub-Navigation inside Client Portal */}
              <div className="grid grid-cols-3 gap-1.5 bg-elegant-sub/60 p-1.5 rounded-2xl border border-elegant-border">
                <button
                  type="button"
                  onClick={() => setPortalSubTab("citas")}
                  className={`py-2 px-1 text-center rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                    portalSubTab === "citas" && clientTab !== "memberships"
                      ? "bg-elegant-card text-amber-300 border border-amber-500/30 shadow-xs"
                      : "text-neutral-400 hover:text-white"
                  }`}
                >
                ...
                <button
                  ...
                  onClick={() => {
                    setClientTab("account");
                    setPortalSubTab("memberships");
                  }}
                  className={`py-2 px-1 text-center rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                    portalSubTab === "memberships" || clientTab === "memberships"
                      ? "bg-elegant-card text-amber-300 border border-amber-500/30 shadow-xs"
                      : "text-neutral-400 hover:text-white"
                  }`}
                >
                  <Award className="h-3.5 w-3.5 shrink-0 text-amber-400" />
                  <span className="truncate">Membresía & Beneficios</span>
                </button>
                <button
                  type="button"
                  onClick={() => {
                    setClientTab("account");
                    setPortalSubTab("loyalty");
                  }}
                  className={`py-2 px-1 text-center rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                    portalSubTab === "loyalty" && clientTab !== "memberships"
                      ? "bg-elegant-card text-amber-300 border border-amber-500/30 shadow-xs"
                      : "text-neutral-400 hover:text-white"
                  }`}
                >
                  <Gift className="h-3.5 w-3.5 shrink-0 text-amber-400" />
                  <span className="truncate">Fidelidad & Ahorro</span>
                </button>
              </div>

              {/* VIEW 1: Membresías & Beneficios (Inside Client Portal) */}
              {(portalSubTab === "memberships" || clientTab === "memberships") && (
                <div className="space-y-6 animate-fadeIn pt-1">
                  ...
                  <div className="space-y-2">
                    ...
                    {loggedClient.membershipActive && loggedClient.membershipId ? (
                        ...
                        return (
                          <div className="bg-gradient-to-br from-elegant-sub to-elegant-card border border-emerald-500/40 p-5 rounded-2xl relative overflow-hidden space-y-3">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-elegant-text-muted text-[11px] font-mono">Socio ID: {loggedClient.id.slice(0, 8)}</span>
                            </div>
                            <div>
                            ...
                            <div className="flex items-center gap-2 pt-2 border-t border-elegant-border/50 text-xs text-emerald-300">
                              ...
                              <span>Disfrutas de un <strong>{plan.discountPercent}% de descuento automático</strong> en todas tus reservas.</span>
                            </div>
                          </div>
                        );
                      })()
                      ...
                      <div className="bg-elegant-sub/40 border border-elegant-border p-5 rounded-2xl text-center space-y-3">
                        ...
                        <p className="text-xs text-elegant-text max-w-md mx-auto">
                          Actualmente no tienes ninguna membresía activa. Explora los planes a continuación y únete al Club de Socios para obtener descuentos automáticos en todos tus servicios.
                        </p>
                      </div>
                    )}
                  </div>

                  {/* All Membership Plans Grid */}
                  <div className="space-y-3">
                    <div className="text-left space-y-1">
                      <h3 className="text-base font-bold text-white">Todos los Planes de Membresía</h3>
                      <p className="text-xs text-elegant-text-muted">
                        Descuentos aplicados directamente en caja y en el agendamiento online.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-1">
                      {memberships.map((plan) => {
                        ...
                        return (
                          <div 
                            key={plan.id}
                            className={`rounded-2xl sm:rounded-3xl p-5 border flex flex-col justify-between relative overflow-hidden transition-all duration-300 ${
                              isMyPlan 
                          ...
                          >
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="space-y-1">
                                ...
                                <p className="text-[11px] text-elegant-text-muted leading-relaxed">{plan.description}</p>
                              </div>

                              {/* Cost */}
                              <div className="pt-2 border-t border-elegant-border/50 flex items-baseline gap-1">
                                <span className="text-xl font-extrabold text-white font-mono">{formatPrice(plan.monthlyPrice)}</span>
                                <span className="text-elegant-text-muted text-[10px]">/ mes</span>
                              </div>

                              {/* Benefit list */}
                              <ul className="space-y-2 text-xs">
                                <li className="flex items-center gap-2 text-emerald-400 font-semibold bg-emerald-950/20 px-2 py-1.5 rounded-xl border border-emerald-900/40 text-[11px]">
                                  <Percent className="h-3.5 w-3.5 shrink-0" />
                                  <span>{plan.discountPercent}% OFF en todo el catálogo</span>
                                </li>
                                {plan.benefits.map((perk, idx) => (
                                  <li key={idx} className="flex items-start gap-1.5 text-elegant-text text-[11px]">
                                    <Check className="h-3 w-3 text-elegant-gold shrink-0 mt-0.5" />
                                    <span>{perk}</span>
                                  </li>
                                ))}
                              </ul>
                            ...
                            {/* Actions */}
                            <div className="pt-4 mt-4 border-t border-elegant-border/40 text-center">
                              {isMyPlan ? (
                                <div className="bg-emerald-950/40 border border-emerald-800/40 text-emerald-400 py-2.5 px-3 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5">
                                  <Check className="h-4 w-4 shrink-0" />
                                  <span>Plan Activo</span>
                                </div>
                              ) : (
                                <div className="bg-elegant-sub/40 border border-elegant-border/50 text-elegant-text-muted py-2.5 px-3 rounded-xl text-[10px] leading-snug">
                                  <span>Solicítalo al administrador en sucursal</span>
                                </div>
                              )}
                            </div>
                          </div>
                  ...
                  {/* Info Box */}
                  <div className="bg-elegant-card border border-elegant-border p-4 rounded-2xl text-xs text-elegant-text-muted flex items-start gap-3">
                    <Gift className="h-5 w-5 text-elegant-gold shrink-0 mt-0.5" />
                    <div className="space-y-1">
                      <p className="font-bold text-white">¿Cómo se aplican tus beneficios de socio?</p>
                      <p className="text-[11px] leading-relaxed">
                        Al agendar turnos con tu cuenta iniciada o usando tu número registrado, el descuento de tu membresía se deduce automáticamente del total. Sin necesidad de ingresar cupones manuales.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {/* VIEW 2: Fidelidad & Ahorro (Inside Client Portal) */}
              {portalSubTab === "loyalty" && clientTab !== "memberships" && (
                <div className="space-y-6 animate-fadeIn pt-1">
                  
                  {/* Indicador de Ahorro Acumulado */}
                  <div className="bg-gradient-to-br from-emerald-950/20 to-neutral-900 border border-emerald-500/20 p-5 rounded-2xl relative overflow-hidden space-y-3 shadow-md">
                    ...
                    <div>
                      ...
                      <h4 className="text-base font-extrabold text-white mt-1.5 font-sans">Ahorro Acumulado de Socio</h4>
                      <p className="text-xs text-elegant-text-muted mt-0.5">
                        Dinero real ahorrado gracias a tu membresía y beneficios en {config.name}:
                      </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between bg-emerald-950/30 border border-emerald-900/30 p-3.5 rounded-xl">
                      ...
                      <div className="text-right">
                        <span className="text-[11px] font-bold text-white block">Beneficio Real</span>
                        <p className="text-[10px] text-emerald-400">¡Tu suscripción rinde frutos! 🎉</p>
                      </div>
                    </div>

                    <div className="text-[10px] text-elegant-text-muted leading-relaxed italic">
                      * Cálculo dinámico entre precios de catálogo y los montos con descuento pagados en tus citas completadas.
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-neutral-900 to-elegant-bg border border-elegant-gold/20 p-5 rounded-2xl relative overflow-hidden space-y-3 shadow-md">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <h4 className="text-base font-extrabold text-white mt-1.5 font-sans">Mi Tarjeta de Sellos</h4>
                        <p className="text-xs text-elegant-text-muted mt-0.5">
                          Completa 5 visitas y obtén un <strong className="text-elegant-gold">Corte o Servicio 100% GRATIS</strong>.
                        </p>
                      </div>
                    ...
                    {/* Reward state */}
                    {(loggedClient.loyaltyPoints || 0) >= 5 ? (
                      <div className="p-3 bg-emerald-950/30 border border-emerald-500/30 rounded-xl flex items-center gap-2 text-xs text-emerald-400">
                        <Gift className="h-4 w-4 shrink-0 text-emerald-400" />
                        <span>¡Felicidades! Tienes tu tarjeta llena. Puedes aplicar tu <strong>Corte Gratis</strong> al agendar tu próximo turno.</span>
                      </div>
                    ) : (
                      <p className="text-[10px] text-elegant-text-muted italic text-center">
                        Te faltan {Math.max(0, 5 - (loggedClient.loyaltyPoints || 0))} visitas completadas para reclamar tu corte gratis.
                      </p>
                    )}
                  </div>
                </div>
              ...
              {/* VIEW 3: Mis Citas & Historial (Inside Client Portal) */}
              {portalSubTab === "citas" && clientTab !== "memberships" && (
                <div className="space-y-4 animate-fadeIn pt-1">
                  ...
                  {(() => {
                    ...
                    if (myUserAppointments.length === 0) {
                      return (
                        <div className="py-8 text-center text-elegant-text-muted text-xs border border-dashed border-elegant-border rounded-2xl space-y-3">
                          <p>Aún no tienes turnos registrados con esta cuenta.</p>
                          <button
                            onClick={() => {
                              setClientTab("booking");
                              setTimeout(() => scrollToStep("step-1-barber"), 100);
                            }}
                            className="px-4 py-2 bg-elegant-gold hover:bg-elegant-gold-hover text-elegant-bg font-bold rounded-xl text-xs inline-flex items-center gap-1.5 cursor-pointer"
                          >
                            <Scissors className="h-3.5 w-3.5" />
                    ...
                    return (
                      <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                              {activeUserApps.map(app => (
                                <div key={app.id} className="p-3.5 bg-elegant-sub border border-amber-500/30 rounded-xl flex items-center justify-between text-xs">
                                  <div className="space-y-1">
                                    <p className="font-bold text-white text-sm">{app.serviceName}</p>
                                    ...
                                    <span className="text-[11px] text-elegant-gold font-semibold">A pagar: {formatPrice(app.price)}</span>
                                  </div>
                                  <div className="text-right space-y-1.5">
                                    ...
                                    <button
                                      onClick={() => handleCancelMyBooking(app.id, app.date, app.time)}
                                      className="text-[10px] text-rose-400 hover:text-rose-300 underline font-semibold block cursor-pointer"
                                    >
                                      Cancelar
                                    </button>
                                  </div>
                        ...
                        {historyUserApps.length > 0 && (
                          <div className="space-y-2 pt-2 border-t border-elegant-border/50">
                              ...
                              {historyUserApps.map(app => (
                                <div key={app.id} className="p-3 bg-elegant-sub/50 border border-elegant-border rounded-xl flex items-center justify-between text-xs opacity-90">
                                  <div className="space-y-1">
                                    ...
                                    {app.hairdresserNotes && (
                                      <div className="mt-1 bg-blue-950/30 p-1.5 rounded text-[10px] text-blue-300 border border-blue-900/30">
                                        <strong>Fórmula:</strong> "{app.hairdresserNotes}"
                                      </div>
                                    )}
                                  </div>
                                  <div>
            ...
            /* Auth Login/Registration Forms & Membership Preview for Guests */
            <div className="space-y-6">
              {/* Guest Sub-Navigation Bar */}
              <div className="max-w-md mx-auto grid grid-cols-2 gap-2 bg-elegant-sub/60 p-1.5 rounded-2xl border border-elegant-border">
                <button
                  type="button"
                  onClick={() => setPortalSubTab("citas")}
                  className={`py-2 px-3 text-center rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                    portalSubTab !== "memberships"
                      ? "bg-elegant-card text-amber-300 border border-amber-500/30 shadow-xs"
                      : "text-neutral-400 hover:text-white"
                  }`}
                >
                  <User className="h-3.5 w-3.5 shrink-0" />
                  <span>Acceso a Mi Cuenta</span>
                </button>
                <button
                  type="button"
                  onClick={() => setPortalSubTab("memberships")}
                  className={`py-2 px-3 text-center rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                    portalSubTab === "memberships"
                      ? "bg-elegant-card text-amber-300 border border-amber-500/30 shadow-xs"
                      : "text-neutral-400 hover:text-white"
              ...
              {portalSubTab === "memberships" ? (
                /* Full Membership Plans for Guests */
                <div className="space-y-6 animate-fadeIn">
                  <div className="text-center max-w-xl mx-auto space-y-2">
                    <span className="bg-elegant-gold/15 text-elegant-gold border border-elegant-gold/30 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider">
                      Club Exclusivo {config.name}
                    </span>
                    <h2 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight">Planes de Membresía & Descuentos</h2>
                    <p className="text-xs text-elegant-text-muted leading-relaxed">
                      Únete como socio para recibir descuentos automáticos en todos tus servicios de corte, barba y tratamientos, además de atención preferencial.
                    </p>
                  </div>
                      ...
                      return (
                        <div
                          ...
                          className={`rounded-3xl p-6 border flex flex-col justify-between relative overflow-hidden transition-all duration-300 ${
                            isVIP
                              ? "bg-gradient-to-b from-elegant-card to-elegant-sub/90 border-elegant-gold/50 shadow-lg shadow-elegant-gold/5 ring-1 ring-elegant-gold/20"
                              : "bg-elegant-card border-elegant-border hover:border-elegant-gold/40 hover:shadow-md"
                          }`}
                        >
                          {isVIP && (
                            <div className="absolute top-0 right-0 bg-elegant-gold text-elegant-bg text-[9px] font-black px-3 py-1 rounded-bl-xl uppercase tracking-wider">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="pt-2 border-t border-elegant-border/50 flex items-baseline gap-1">
                              ...
                              <span className="text-elegant-text-muted text-[11px]">/ mes</span>
                            </div>
                          ...
                          <div className="pt-5 mt-5 border-t border-elegant-border/40 space-y-2">
                            <button
                              type="button"
                              onClick={() => {
                                setPortalSubTab("citas");
                              ...
                              className="w-full py-2.5 bg-elegant-gold hover:bg-elegant-gold-hover text-elegant-bg font-extrabold rounded-xl text-xs flex items-center justify-center gap-1.5 cursor-pointer transition-colors shadow-xs"
                            >
                              <span>Crear Cuenta para Unirme</span>
                              <ArrowRight className="h-3.5 w-3.5" />
                            </button>
                            <p className="text-[10px] text-elegant-text-muted text-center">
                              Activación en caja o con el administrador
                            </p>
                          </div>
                        </div>
                      );
                  ...
                  <div className="bg-elegant-card border border-elegant-border p-4 rounded-2xl max-w-xl mx-auto text-xs text-elegant-text-muted flex items-start gap-3">
                    <Gift className="h-5 w-5 text-elegant-gold shrink-0 mt-0.5" />
                    <div className="space-y-1">
                      <p className="font-bold text-white">¿Cómo funcionan los beneficios?</p>
                      <p className="text-[11px] leading-relaxed">
                        Solo necesitas crear tu cuenta gratuita. Al contratar tu membresía con nosotros, tus descuentos se calcularán de manera 100% automática cada vez que reserves desde esta web o en la app móvil.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                /* Auth Form View */
                <div className="bg-elegant-card border border-elegant-border rounded-3xl p-4 sm:p-6 space-y-6 shadow-xs max-w-xl mx-auto">
                  
                  {/* Form Toggles */}
                  <div className="flex border-b border-elegant-border pb-1 gap-1">
                    <button
                      type="button"
                      onClick={() => {
                        setAuthMode("login");
                      ...
                      className={`flex-1 py-2 font-sans font-bold text-xs sm:text-sm transition-all border-b-2 text-center cursor-pointer ${
                        authMode === "login"
                  ...
                  {/* Teaser de Membresías para usuarios invitados */}
                  <div className="pt-4 border-t border-elegant-border space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5 text-xs font-bold text-amber-300">
                        <Award className="h-4 w-4 text-amber-400" />
                        <span>Beneficios del Club de Socios</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => setPortalSubTab("memberships")}
                        className="text-[10px] text-elegant-gold font-bold hover:underline cursor-pointer"
                      >
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {memberships.slice(0, 3).map((plan) => (
                        <div key={plan.id} className="p-2.5 bg-elegant-sub/60 border border-elegant-border rounded-xl text-center space-y-1">
                          <p className="text-[10px] font-extrabold text-white">{plan.name}</p>
                          <p className="text-[11px] font-black text-amber-400 font-mono">{plan.discountPercent}% OFF</p>
                          <p className="text-[9px] text-elegant-text-muted">{formatPrice(plan.monthlyPrice)}/mes</p>
                        </div>
                      ))}
                    </div>
                    <p className="text-[10px] text-elegant-text-muted text-center leading-relaxed">
                      Crea tu cuenta de socio gratuita para acumular puntos de fidelidad, sellos para cortes gratis y descuentos de membresía.
                    </p>
                  </div>
                </div>
              )}
      ...
      {/* Sticky Mobile Booking Summary & Action Bar (Visible only on mobile/tablet when in booking tab) */}
      {clientTab === "booking" && !bookingSuccess && (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-neutral-950/95 backdrop-blur-md border-t border-amber-500/30 p-3 sm:px-6 shadow-2xl animate-slideUp">
          <div className="max-w-md mx-auto flex items-center justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5 text-[11px] text-amber-400 font-bold truncate">
                <span>💈 {selectedService ? selectedService.name : "Elige tu servicio"}</span>
              </div>
              <p className="text-[10px] text-neutral-400 truncate">
                {selectedDate && selectedTime ? (
                  <span>📅 {formatDateLabel(selectedDate).dayName.slice(0, 3)} {formatDateLabel(selectedDate).dayNum} a las {formatTime(selectedTime, config?.timeFormat)}</span>
                ) : (
                  <span>🕒 Selecciona horario y barbero</span>
                )}
              </p>
              {selectedService && (
                <p className="text-xs font-black font-mono text-white">
                  Total: {formatPrice(
                    (useBirthdayBenefit || redeemReward) 
                      ? 0 
                      : (discountPercent 
                          ? Math.round(selectedService.price * (1 - discountPercent / 100)) 
                          : selectedService.price)
            ...
            <div className="shrink-0">
              {(!selectedDate || !selectedTime) ? (
                <button
                  type="button"
                  onClick={() => scrollToStep("step-2-datetime")}
                  className="px-4 py-2.5 bg-amber-500 hover:bg-amber-400 text-black font-extrabold text-xs rounded-xl transition-all shadow-md flex items-center gap-1 cursor-pointer active:scale-95"
                >
                ...
                <button
                  type="button"
                  onClick={() => scrollToStep("step-4-contact")}
                  className="px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold text-xs rounded-xl transition-all shadow-md flex items-center gap-1 cursor-pointer active:scale-95"
                >
                  <span>Tus Datos (Paso 4)</span>
                  <ChevronRight className="h-3.5 w-3.5" />
                </button>
              ) : (
                <button
                  type="button"
                  onClick={() => scrollToStep("step-4-contact")}
                  className="px-4 py-2.5 bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-black font-black text-xs rounded-xl transition-all shadow-lg shadow-amber-500/20 flex items-center gap-1.5 cursor-pointer active:scale-95 animate-pulse"
                >
